...
- ロックステップ動画レンダリング（WebCodecs + FFmpeg Mux）を実装
  - 決定論的CFRでの高品質H.264出力（GOP制御、Annex B → MP4 mux）
  - 背景動画の事前フレーム抽出（FFmpeg）とタイムライン整合の仕組み
  - ステップ別の進行度・ETA 表示（背景準備/メイン処理/最終mux）

- GPUメモリリーク対策・安定化
  - オフスクリーンRenderTextureプールの初期化/破棄の徹底
  - WebGL `gl.finish()`/`gl.flush()` の適用タイミング見直しと短い待機を挿入
  - 背景動画はシーク後にフリーズさせ、不要なCALayer/サーフェス切替を抑制
  - フレームキャプチャ中の一時的GC促進と小刻みな間隔挿入で安定化

- テンプレートと合成パイプラインのバグ修正/改善
  - 背景/前景のスケール・パディング方式を統一してアスペクト比の一貫性を確保
  - バッチ動画作成と最終結合の解像度/AR検証ログ追加、抜け漏れ検出の強化
  - 一部縦書き・黒帯系テンプレートの安定性向上

- 進行度UIの改善
  - 出力ボタンの上に進行度バー・ステップ表示・ETAを表示
  - 背景準備ステップの進捗をFFmpeg出力から間欠取得して可視化
...
#### 実験的機能
- **ビート検出システム**: 音楽のリズムに同期したアニメーション制御（統合済み）
  - BeatDetector: オンセット検出アルゴリズム
  - AudioAnalyzer: FFT解析と音声処理
  - BeatManager: 再生・シークに追従するビートイベント管理
  - コンテンツタブでの検出設定とタイムライン上のビートマーカー表示
- **キネティック・シーンコンポーザー**: レイアウト／出現／継続／消失／画面モーションを組み合わせる決定論的テンプレート
  - `sequence`、`parallel`、`delay`、`repeat` と共通イージングをTypeScriptから利用可能
  - 同一時刻・同一seedに対して常に同じ描画結果を生成
